	...
	<div id="content">
		...
		<div class="main-content">
			<div class="row">
						...
						<div class="table-title">
							<div class="row">
								<div class="col-sm-6 p-0 flex justify-content-lg-start justify-content-center">
									<h2 class="ml-lg-2 text-light">Modelo de vehiculo</h2>
								</div>
								<div class="col-sm-6 p-0 flex justify-content-lg-end justify-content-center">
									<a href="#addEmployeeModal" class="btn btn-success" data-toggle="modal" onclick="crear_modelo()">
				...
				<form action="Controlador/Modelo.php" method="post">
							...
							<div class="modal-content">
								...
								<div class="modal-body">
									<div class="form-group">
										...
										<input type="text" name="Nombre" id="nombre" class="form-control" title="Solo puedes ingresar caracteres alfanumericos" minlength="3" maxlength="15" required>
									</div>
		...
		<script type="text/javascript">
			$("#nombre").on("input", function() {
				this.value = this.value.replace(/[^0-9-a-z-A-ZÀ-ÿ\u00f1\u00d1 ]/g, '');
			})
			document.getElementById("nombre").addEventListener("keyup", async (e) => {
				if (e.target.value.length >= 4) {
					await fetch(`Controlador/Modelo.php?operacion=ConsultarModelo&&nombre=${e.target.value}`)
			...
			$(document).ready(() => {
				...
				$("#tabla").DataTable({
					"ajax": {
						"url": "Controlador/Modelo.php?operacion=ConsultarTodos",
						"dataSrc": "data"
					},
					"columns": [{
							data: "ID"
			...
			*/
			const consultarUno = async (id) => {
				...
				await fetch("Controlador/Modelo.php?operacion=ConsultarUno&&id=" + id)
					.then(response => response.json())
					.then(({
						data
					}) => {
						$("#id").val(data.ID)
						$("#nombre").val(data.modelo_Nombre)
