...
class Producto_m extends bd
{
  ...
  {
    $sql = "INSERT INTO producto(producto_Nombre,producto_Estatus,producto_Fecha) VALUES (UPPER('$this->nombre'),true,NOW())";
  ...
  {
    $res = $this->ejecutar("SELECT * FROM movimiento WHERE ID_Producto = $this->id");
    $res = $res->fetch_assoc();
    if ($res != "" || $res != null) {
      return false;
    } else {
  ...
  {
    $res = $this->ejecutar("SELECT * FROM producto WHERE producto_Nombre = '$nombre'");
    if ($res) $res = $res->fetch_assoc();
    else $res = ["SELECT * FROM producto WHERE producto_Nombre = '$nombre'"];
    return $res;
  }
